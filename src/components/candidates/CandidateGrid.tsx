...
// Kode dan Nama Wadah tetap (ketetapan)
const KODE_NAMA_WADAH = "KB00230015 / JOHAN SILAEN YOHANDA";

// Ketetapan nama & jabatan penanda tangan
const NAMA_WADAH    = "HERDIANA SIMBOLON";
const JABATAN_WADAH = "KETUA WADAH";
const NAMA_CABANG   = "SAKINAH RAMZA";
const JABATAN_CABANG = "ARK";

function fixRoman(str: string): string {
  return str.replace(/\b([ivxlcdm]+)\b/gi, (match) => {
...
async function buildPayload(
  c: Candidate,
  nomorSurat?: number
): Promise<KeplingPayload> {
  // Lookup kodepos dari kelurahan + kecamatan calon perisai
  // kelDesa = kelurahan, kecamatan = kecamatan
  const kodePos = await lookupKodePos(c.kelDesa || c.subDistrict, c.kecamatan);
  ...
  return {
    ...
    kabupatenKota:    "MEDAN",
    kodePos,
    kelurahan:        c.kelDesa,      // untuk fallback lookup backend
    kecamatan:        c.kecamatan,    // untuk fallback lookup backend
    noTelp:           c.phone,
    email:            c.email,
    pekerjaan:        "PERISAI",
    bank:             c.rekening,
    ...
    jabatanWadah:     JABATAN_WADAH,
    namaCabang:       NAMA_CABANG,
    jabatanCabang:    JABATAN_CABANG,
    urlGDriveFolder:  c.documents.kumpulanBerkas || undefined,
    nomorSurat,
  };
}
...
function isReadyToPrint(c: Candidate): boolean {
  return (
    !isDone(c) &&
...
function isIncomplete(c: Candidate): boolean {
  return (
    !isDone(c) &&
    !c.readyToPrint?.form &&
    !c.readyToPrint?.pks &&
        ...
        return (
          <button
            ...
            style={{
              ...
              gap: "10px",
              padding: "9px 12px",
              background: active ? "#F2FCFA" : "transparent",
              border: "none",
    ...
    >
      <div
        style={{
          ...
          height: "16px",
          borderRadius: "8px",
          background: active ? "#16A08F" : "#CBD5E0",
...
export function CandidateGrid({
  ...
  useEffect(() => {
    if (
      kelurahanTabs.length > 0 &&
      (!activeTab || !kelurahanTabs.includes(activeTab))
    ) {
      setActiveTab(kelurahanTabs[0]);
    }
  }, [kelurahanTabs, activeTab]);

  // Notif badge per kelurahan
  ...
  const visible = useMemo(() => {
    ...
    return [...filtered].sort((a, b) => {
      const score = (c: Candidate) => {
        if (isReadyToPrint(c)) return 0;
        if (isDone(c))         return 2;
        return 1;
      };
  ...
  async function handlePrintDoc(candidateId: string, doc: keyof PrintedDocs) {
    ...
    try {
      ...
      alert(
        `Gagal generate preview: ${err instanceof Error ? err.message : String(err)}\n\nPastikan backend Python sudah berjalan di http://127.0.0.1:8000`
      );
    } finally {
  ...
  async function handlePreviewConfirm(doc: keyof PrintedDocs) {
    ...
    try {
      ...
      alert(
        `Gagal download: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  ...
  async function handleMerge(candidateId: string) {
    const candidate = candidates.find((c) => c.id === candidateId);
    if (!candidate || !candidate.documents.kumpulanBerkas) {
      alert("Folder GDrive tidak ditemukan untuk kandidat ini.");
      return;
    }
    try {
      await generateMerged(
        candidate.documents.kumpulanBerkas,
        candidate.namaCalonPerisai || candidate.namaKepling
      );
    } catch (err: unknown) {
      alert(
        `Gagal merge files: ${err instanceof Error ? err.message : String(err)}`
      );
    }
  }

  const filterLabel = (() => {
    if (activeFilters.size === 0) return "Filters";
    const labels = Array.from(activeFilters).map(
      (k) => FILTERS.find((f) => f.key === k)!.label
    );
  ...
  return (
    <section>
      ...
      >
        ...
        >
          <span style={{ fontWeight: 600 }}>{visible.length}</span> perisai
          candidates
        </p>
          ...
          <div style={{ position: "relative" }}>
            <button
              ...
              style={{
                ...
                background: activeFilters.size > 0 ? "#EEF9F7" : "#FFFFFF",
                display: "flex",
                alignItems: "center",
                gap: "5px",
                cursor: "pointer",
            ...
            >
              <SlidersHorizontal
                size={12}
                color={activeFilters.size > 0 ? "#16A08F" : "#4A5255"}
                style={{ flexShrink: 0 }}
              />
              <span
                style={{
